...
\begin{itemize}
    ...
    \item \textbf{Inteligencia Artificial:} Ecosistema LangChain (LangChain, LangGraph y LangSmith) operando con la API de OpenAI (Modelo \texttt{gpt-4o-mini}).
...
\section{El Ecosistema de Inteligencia Artificial}
Para dotar a la plataforma de verdadera autonomía, el sistema emplea una tríada tecnológica de última generación que conforma el Ecosistema LangChain. La combinación de estas tres herramientas permite no solo crear al agente, sino monitorearlo y dotarlo de razonamiento lógico continuo:

\vspace{0.5cm}
\begin{center}
\makebox[\textwidth][c]{
\begin{tikzpicture}[node distance=1cm and 1.5cm, align=center]
    % LangChain
    \node (langchain) [process, fill=blue!10, text width=4.5cm, minimum height=4cm] {
        \textbf{LangChain} \\
    ...
    % LangGraph
    \node (langgraph) [process, fill=orange!10, text width=4.5cm, minimum height=4cm, right=1.5cm of langchain] {
        \textbf{LangGraph} \\
        (Orquestación) \\
        \vspace{0.2cm}
    ...
    % LangSmith
    \node (langsmith) [process, fill=green!10, text width=4.5cm, minimum height=4cm, right=1.5cm of langgraph] {
        \textbf{LangSmith} \\
        (Observabilidad) \\
        \vspace{0.2cm}
        - Monitoreo en Tiempo Real \\
        - Traza de Tokens (Logs) \\
        - Depuración y Evaluación
    };

    % Arrows
    \draw [arrow] (langchain) -- node[above] {Orquestación} (langgraph);
    \draw [arrow] (langgraph) -- node[above] {Telemetría} (langsmith);
    \draw [arrow] (langchain) edge[bend left=20] node[above] {Telemetría Directa} (langsmith);
\end{tikzpicture}
}
\end{center}

\begin{itemize}
    \item \textbf{LangChain} provee los bloques de construcción fundamentales para hablar con el modelo de lenguaje (LLM).
    \item \textbf{LangGraph} toma esos bloques y los introduce en un motor de grafos cíclicos, dándole al agente la memoria y la lógica necesarias para crear un ciclo \textit{ReAct} (Razonar $\rightarrow$ Actuar $\rightarrow$ Repetir) sin quedarse atrapado en bucles infinitos.
    \item \textbf{LangSmith} actúa como la caja negra del avión, registrando cada paso lógico del agente y el consumo de tokens para realizar auditorías de calidad de las respuestas.
\end{itemize}
...
El núcleo algorítmico que hace posible este ciclo de razonamiento se implementa a través de LangGraph de la siguiente forma:

\begin{lstlisting}[language=Python, caption=Implementación de Arquitectura ReAct con LangGraph]
from langgraph.prebuilt import create_react_agent
from langchain_openai import ChatOpenAI
from langchain_experimental.tools import PythonAstREPLTool

# 1. Herramienta (Acción: Acceso a la base de datos)
tool = PythonAstREPLTool(locals={"df": df_filtered})

# 2. Cerebro (Pensamiento: Modelo Lógico)
llm = ChatOpenAI(temperature=0, model="gpt-4o-mini", api_key=api_key)

# 3. Orquestador (Memoria y Ciclo Multi-paso de LangGraph)
agent_executor = create_react_agent(llm, [tool])

# 4. Invocación Autónoma
