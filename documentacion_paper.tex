...
\begin{tikzpicture}[node distance=1.5cm and 1.5cm]
    ...
    \node (sqlite) [database, right=1.5cm of pandas, text width=2cm, text centered] {Almacenamiento (SQLite)};
    
    \node (ui) [process, below=1.5cm of sqlite, text width=3cm] {Visualización Reactiva (Plotly)};
    \node (agent) [startstop, below=1.5cm of pandas, text width=3cm] {Agente de IA (Análisis Ad-Hoc)};
    ...
    \draw [arrow] (sqlite) |- node[anchor=north east] {Lectura Contextual} (agent);
    \draw [arrow, <->] (ui) -- node[anchor=north] {Interacción UI} (agent);
\end{tikzpicture}
}
\end{center}
