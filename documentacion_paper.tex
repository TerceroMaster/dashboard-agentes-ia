...
\begin{tikzpicture}[node distance=2.5cm and 3cm]
    ...
    \node (ui) [process, below=1cm of user] {Interfaz Streamlit (Chat)};
    \node (langchain) [io, below=1cm of ui] {LangChain Orquestador};
    
    \node (llm) [process, right=2.5cm of langchain] {OpenAI (GPT-4o-mini)};
    \node (python) [process, below=1.5cm of langchain] {Python REPL (Pandas)};
    
    \node (db) [database, left=2cm of python] {SQLite / DataFrame};
    
    \node (output) [process, below=1.5cm of python] {Generación de Archivos (Excel/PDF)};
    \node (response) [startstop, below=1.5cm of output] {Respuesta al Usuario};

    % Flechas
    \draw [arrow] (user) -- (ui);
    \draw [arrow] (ui) -- (langchain);
    \draw [arrow] (langchain) -- node[anchor=south] {Prompt} (llm);
    \draw [arrow] (llm) |- node[anchor=west, pos=0.75] {Código Python} (python);
    ...
    \draw [arrow] (langchain) edge[bend right=50] node[anchor=east] {Texto Final} (response);
